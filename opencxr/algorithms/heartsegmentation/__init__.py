...
class HeartSegmentationAlgorithm(BaseAlgorithm):
    def __init__(self):
        ...
        if not os.stat(path_to_model_resolved).st_size > 150000000: # model file needs to be larger than 150MB or it was not pulled correctly
            print(
                "Heart segmentation model file seems incorrect (size ", os.stat(path_to_model_resolved) , ") downloading the weights......",
                path_to_model_resolved
            )
            os.remove(path_to_model_resolved) # first delete the file, because wget will not overwrite
            file_url = "https://github.com/DIAGNijmegen/opencxr/tree/master/opencxr/algorithms/model_weights/heart_seg.h5"
            wget.download(file_url, path_to_model_resolved)
            print('file now exists?', os.path.isfile(path_to_model_resolved))
            print('file size now', os.stat(path_to_model_resolved))
    ...
    def process_image(self, input_image):
        """
        Calls the model to obtain heart segmentation
        :param input_image: input np array 512x512 y,x ordering
        :return: np array 512x512 of heart segmentation
        """
    ...
    def run(self, image):
        """
        The call to run heart segmentation on an image
        ...
        # if the seg map has nothing segmented then later rescaling operations will fail so we return immediately in that case
        if np.max(seg_map) == 0:
            return np.zeros(orig_img_shape).astype(np.uint8)

        # resize the segmentation (size 512) to the same size as original input using record of original size changes
        seg_original = self.resize_to_original(seg_map, size_changes)
        # tidy up by removing holes and keeping largest connected component
