...
if result['Type'] == expected_result['Type'] and \
    result['Rotation'] == expected_result['Rotation'] and \
    result['Inversion'] == expected_result['Inversion'] and \
    result['Lateral_Flip'] == expected_result['Lateral_Flip']:
...
if np.array_equal(np.round(np.asarray(result['Type_Probs_PA_AP_lateral_notCXR']), 3), np.round(np.asarray(expected_result['Type_Probs_PA_AP_lateral_notCXR']), 3)) and \
   np.array_equal(np.round(np.asarray(result['Rotation_Probs_0_90_180_270']), 3), np.round(np.asarray(expected_result['Rotation_Probs_0_90_180_270']), 3)) and \
    np.array_equal(np.round(np.asarray(result['Inversion_Probs_No_Yes']), 3), np.round(np.asarray(expected_result['Inversion_Probs_No_Yes']), 3)) and \
    np.array_equal(np.round(np.asarray(result['Lateral_Flip_Probs_No_Yes']), 3), np.round(np.asarray(expected_result['Lateral_Flip_Probs_No_Yes']), 3)):
