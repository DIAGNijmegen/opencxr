...
class Normalizer():
    ...
    @classmethod
    def __split_energy_bands(cls, image, sigmas):
    ...
    @classmethod
    def __report_energy_bands(cls, bands, mask=1):
        means = []
        stdevs = []
        shape = bands[0].shape
    ...
    @classmethod
    def __reconstruct(cls, bands, means, stdevs, coefficients, lung_mean=None, mediastinum_mean=None):
    ...
    @classmethod
    def __report_energy_lungs_and_mediastinum(cls, band, mean, stdev, mask):
        # Not used at present
        band = band - mean
        band = band / stdev
        lung_values = []
    ...
    @classmethod
    def __get_norm_central_70(cls, img_np, spacing):
        """
        ...
        bands = cls.__split_energy_bands(img_np, sigmas)
        # get means and stddevs of the energy bands
        means, stdevs = cls.__report_energy_bands(bands)
        # reconstruct an image from the energy bands and the reference values
        norm_70 = cls.__reconstruct(bands, means, stdevs, rick_coeffs70)
    ...
    @classmethod
    def __get_norm_lung_mask(cls, img_np_norm70, lung_mask_np):
        """
        ...
        sigmas = [1, 2, 4, 8, 16]
        # split into energy bands
        bands = cls.__split_energy_bands(img_np_norm70, sigmas)
        # get means and stddevs of the energy bands
        means, stdevs = cls.__report_energy_bands(bands, mask=lung_mask_np)
        ...
        norm = cls.__reconstruct(bands, means, stdevs, rick_coeffs_lungseg)#omit for now:, lung_mean, mediastinum_mean)
